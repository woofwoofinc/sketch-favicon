...
buildah run $IMAGE -- apt-get install -qq zip
...
buildah run $IMAGE -- apt-get install -qq pkg-config libsecret-1-dev
buildah run $IMAGE -- /usr/bin/yarn global add skpm
